...
function FaceButton(){
	...
	oImgFace.border = 0;
	oImgFace.src = "./src/img/smile.gif";
...
function GameOver(result){
	switch(result)
	{
		...
		case 1:
			ExpandAll();
			oImgFace.src = "./src/img/blast.gif";
...
function CheckConfigAndStart() {
	...
	if(num > (row*col/2)) {
		...
		document.frmConfig.mine_num.value = row*col/2;
		return false;
	}
	if(num < 8) {
		alert("Too few mines!");
