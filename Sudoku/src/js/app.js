...
document.addEventListener('DOMContentLoaded', function() {
    ...
    id("home-btn").addEventListener("click", home_btn);
...
function startGame() {
    ...
    } else if (id("difficulty-custom").checked) {
        set_difficulty = Number(id("difficulty-custom").value);
        inputBoard = generateSudoku(Number(id("difficulty-custom").value));
    }
    // Initialize game with the given inputBoard
    id("game-control").style.display = '';
...
function endGame() {
    ...
    if (timerType == "countdown") {
        ...
        if (lives == 0 || (parseInt(m, 10) == 0 && parseInt(s, 10) == 0)) {
            ...
            var audio = new Audio('./src/audio/audio-lose.wav');
            title_txt = "GAME OVER.😮";
        } else {
            var x = id("snackbar-win");
            var audio = new Audio('./src/audio/audio-win.wav');
        }
    } else if (timerType == "stopwatch") {
...
function displayLives(lives) {
    id("lives").innerHTML = "<i class=\"fas fa-heart\" style=\"color: #d70f0f; margin: 0px 5px;\"></i>".repeat(lives) + "<i class=\"fas fa-heart-broken\" style=\"color: #d70f0f; margin: 0px 5px;\"></i>".repeat(MAX_LIVES - lives);
}
...
function refresh_puzzle() {
    ...
    resetGame();
    // Choose board difficulty and initialize the Sudoku board accordingly
    inputBoard = generateSudoku(set_difficulty);
    // Initialize game with the given inputBoard
    initializeGame(inputBoard);
}
