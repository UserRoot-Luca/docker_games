<!-- Original: https://github.com/ccprog/bu-patience -->
<?php
    ...
    $cards_version = file_get_contents("src/js/cards_version");
    $dataset = 'data-cards_version="' . $cards_version . '" ';

    $rules = scan_options('src/rulesets', 'ruleset', function ($dir, $entry, $key) {
    ...
    $languages = scan_options('src/lang', 'language', function ($dir, $entry, $key) {
        return array(
            "file" => $dir . "/" . $entry,
            "read" => $key
        );
...
<html dir="ltr" charset="utf-8">
    <head>
        ...
        <script type="application/ecmascript" src="src/js/patience.js" ></script>
        <link rel="stylesheet" type="text/css" href="lib/bootstrap/css/bootstrap.css" />
        <link rel="stylesheet" type="text/css" href="src/css/patience.css" />
    </head>
    <body <?php echo $dataset ?>>
        <div id="page">
            <div id="controls" class="">
                <select id="language" class="form-control" title="Language">
                    <?php 
                        foreach($languages['items'] as $key => $value) { ?>
                            <option <?php if($key == $languages['selected']) echo 'selected="selected" '; ?> value="<?php echo $value['file'] ?>"><?php echo $value["read"]; ?></option> 
                    <?php } ?>
                </select>
                <select id="ruleset" class="form-control"> 
                    <?php 
                        foreach($rules['items'] as $key => $value) { ?>
                            <option <?php if($key == $rules['selected']) echo 'selected="selected" '; ?> value="<?php echo $value['file'] ?>"><?php echo $value["read"]; ?></option> 
                    <?php } ?>
                </select>
